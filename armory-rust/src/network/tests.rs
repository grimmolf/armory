...
use bitcoin::{Network, Amount, Address, Transaction};
...
mod peer_communication_tests {
    ...
    async fn test_peer_connection_states() {
        ...
        assert!(!client.is_connected());
        assert!(matches!(client.connection_status(), ConnectionState::Disconnected));
...
mod rpc_tests {
    ...
    async fn test_rpc_failover() {
        ...
        assert_eq!(client.current_endpoint().url.as_str(), "http://localhost:18443/");
...
mod integration_tests {
    ...
    async fn test_multi_endpoint_rpc() {
        ...
        let mut client = RpcClient::new(
            vec![endpoint1, endpoint2, endpoint3], 
            Network::Regtest
        ).unwrap();
