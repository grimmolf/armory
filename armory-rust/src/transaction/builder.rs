...
use crate::transaction::psbt::{PsbtV2, PsbtV2Input, PsbtV2Output};
use crate::wallet::descriptor_wallet::{Wallet, Utxo, AddressType};
use bitcoin::{
    Address, Amount, ScriptBuf, TxOut, Transaction,
    FeeRate, Txid, 
    absolute::LockTime,
    transaction::Version,
};
...
impl TransactionBuilder {
    ...
    pub fn with_config(wallet: Arc<RwLock<Wallet>>, config: BuilderConfig) -> TransactionResult<Self> {
        Ok(Self {
            wallet,
            psbt: PsbtV2::new()?,
            outputs: Vec::new(),
    ...
    pub fn add_recipient(&mut self, address: Address, amount: Amount) -> TransactionResult<&mut Self> {
        if amount == Amount::ZERO {
            return Err(TransactionError::InvalidAmount("Cannot send zero amount".to_string()));
        }
    ...
    pub fn estimate_fee(&mut self) -> TransactionResult<Amount> {
        ...
        // Calculate fee
        let fee = fee_rate.fee_vb(tx_size as u64)
    ...
    pub fn select_utxos(&mut self) -> TransactionResult<&mut Self> {
        ...
        let total_output = self.outputs.iter()
            .try_fold(Amount::ZERO, |acc, (_, amount)| {
        ...
        let available_utxos = {
            let wallet = self.wallet.read().map_err(|_| TransactionError::InvalidInput("Failed to read wallet".to_string()))?;
        ...
        let (selected_utxos, change_amount) = self.perform_coin_selection(
            &available_utxos, 
            total_output, 
            estimated_fee
        )?;
        ...
        if change_amount > Amount::ZERO {
            let change_address = {
                let mut wallet = self.wallet.write().map_err(|_| TransactionError::InvalidInput("Failed to write wallet".to_string()))?;
                wallet.get_change_address(AddressType::NativeSegwit)
                    .map_err(|e| TransactionError::InvalidInput(format!("Failed to get change address: {}", e)))?
            };
    ...
    pub fn build_psbt(&mut self) -> TransactionResult<PsbtV2> {
        ...
        for utxo in &self.selected_utxos {
            self.psbt.add_input(utxo.txid, utxo.vout, self.get_sequence_number())?;
        ...
        if let Some((change_address, change_amount)) = &self.change_output {
            self.psbt.add_output(*change_amount, change_address.script_pubkey())?;
        }
    ...
    pub fn total_output_value(&self) -> Amount {
        let recipient_total = self.outputs.iter()
            .fold(Amount::ZERO, |acc, (_, amount)| acc + *amount);
        
        let change_amount = self.change_output
            .as_ref()
            .map(|(_, amount)| *amount)
    ...
    fn get_fee_rate(&self) -> TransactionResult<FeeRate> {
        match self.config.fee_strategy {
            ...
            FeeStrategy::Normal => {
                FeeRate::from_sat_per_vb(20)
                    .ok_or_else(|| TransactionError::FeeEstimation("Invalid fee rate".to_string()))
            }
            FeeStrategy::HighPriority => {
                FeeRate::from_sat_per_vb(50)
                    .ok_or_else(|| TransactionError::FeeEstimation("Invalid fee rate".to_string()))
            }
        }
    }
    ...
    fn perform_coin_selection(
        ...
        for utxo in sorted_utxos {
            ...
            total_selected = total_selected.checked_add(Amount::from_sat(utxo.value))
                .ok_or_else(|| TransactionError::InvalidAmount("Selected value overflow".to_string()))?;
...
impl FeeEstimator {
    ...
    pub fn estimate_fee(tx_size_vbytes: usize, fee_rate: FeeRate) -> TransactionResult<Amount> {
        fee_rate.fee_vb(tx_size_vbytes as u64)
            .ok_or_else(|| TransactionError::FeeEstimation("Fee calculation overflow".to_string()))
    }
    ...
    pub fn get_fee_rate_for_target(blocks: u32) -> FeeRate {
        ...
        FeeRate::from_sat_per_vb(sat_per_vb).unwrap_or_else(|| {
            FeeRate::from_sat_per_vb(1).expect("Minimum fee rate")
        })
    }
