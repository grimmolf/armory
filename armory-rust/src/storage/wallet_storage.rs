...
use crate::error::{StorageError, StorageResult, WalletError, WalletResult};
use crate::crypto::{SecureKey, encrypt_data, decrypt_data, EncryptedData};
...
impl WalletStorage {
    ...
    pub fn save_wallet_data(&self, data: &WalletData) -> StorageResult<()> {
        ...
        self.db.insert(key.as_bytes(), final_data)
            .map_err(|e| StorageError::Database(e.to_string()))?;
    ...
    pub fn load_wallet_data(&self, wallet_id: &str) -> StorageResult<WalletData> {
        ...
        let stored_data = self.db.get(key.as_bytes())
            .map_err(|e| StorageError::Database(e.to_string()))?
    ...
    fn create_backup(&self, wallet_id: &str) -> StorageResult<()> {
        ...
        let backup_file = backup_dir.join(format!("{}_{}.backup", wallet_id, timestamp));
...
mod tests {
    ...
    fn create_test_wallet_data() -> WalletData {
        ...
        WalletData {
            ...
            encrypted_seed,
            descriptors: vec![
                "wpkh(xpub123...)".to_string(),
                "tr(xpub456...)".to_string(),
            ],
