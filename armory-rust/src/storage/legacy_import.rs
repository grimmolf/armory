...
pub fn import_armory_wallet(
    ...
    let wallet_data = convert_to_modern_format(&header, &root_key_data, &entries, passphrase)?;
...
fn validate_wallet_file(header: &LegacyHeader) -> StorageResult<()> {
    ...
    if header.version < 0x01000000 || header.version > 0x01230000 {
        return Err(StorageError::Database(format!(
            "Unsupported wallet version: {:08x}",
            header.version
        )));
    }
    ...
    loop {
        ...
        match entry_type_val {
            ...
            _ => {
                ...
                return Err(StorageError::Database(format!(
                    "Unknown entry type: {entry_type_val}"
                )));
            }
        }
    }
