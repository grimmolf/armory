...
use crate::crypto::{NONCE_SIZE, generate_random_bytes};
...
pub fn decrypt_data(
    ...
    if encrypted_data.nonce.len() != NONCE_SIZE {
        return Err(CryptoError::Decryption(
            format!("Invalid nonce size: expected {}, got {}", 
                   NONCE_SIZE, encrypted_data.nonce.len())
        ));
    }
...
mod tests {
    ...
    fn test_password_based_encryption() {
        ...
        let encrypted = encrypt_with_password(
            password, salt, plaintext, 1024, 1
        ).unwrap();
        
        let decrypted = decrypt_with_password(
            password, salt, &encrypted, 1024, 1
        ).unwrap();
        ...
        assert!(decrypt_with_password(
            "wrong_password", salt, &encrypted, 1024, 1
        ).is_err());
    }
