...
pub fn generate_random_bytes(length: usize) -> CryptoResult<Vec<u8>> {
    ...
    getrandom(&mut buffer).map_err(|_e| CryptoError::RandomGeneration)?;
    Ok(buffer)
}
...
pub fn generate_private_key_bytes() -> CryptoResult<[u8; 32]> {
    ...
    loop {
        ...
        getrandom(&mut bytes).map_err(|_| CryptoError::RandomGeneration)?;
...
pub fn fill_random(buffer: &mut [u8]) -> CryptoResult<()> {
    getrandom(buffer).map_err(|_| CryptoError::RandomGeneration)?;
    Ok(())
}
