...
use secp256k1::{
    Secp256k1, SecretKey, PublicKey, Message, ecdsa::Signature,
    schnorr, XOnlyPublicKey, Keypair, All
};
...
pub mod schnorr_ops {
    ...
    pub fn sign(private_key: &PrivateKey, message: &[u8]) -> CryptoResult<[u8; 64]> {
        ...
        let aux_rand = [0u8; 32];
        let signature = private_key.context().secp().sign_schnorr_with_aux_rand(&msg, &keypair, &aux_rand);
...
pub mod ecdsa_ops {
    ...
    pub fn sign(private_key: &PrivateKey, message: &[u8]) -> CryptoResult<Vec<u8>> {
        ...
        let signature = private_key.context().secp().sign_ecdsa(&msg, private_key.secret_key());
