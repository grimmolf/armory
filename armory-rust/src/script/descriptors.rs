...
impl ScriptDescriptor {
    ...
    pub fn custom(descriptor: String, network: Network) -> TransactionResult<Self> {
        Self::new(
            DescriptorType::Custom(descriptor.clone()),
            descriptor,
            network,
        )
    }
    ...
    fn parse(&mut self) -> TransactionResult<()> {
        ...
        let parsed: Descriptor<PublicKey> = self.descriptor.parse().map_err(|e| {
            TransactionError::ScriptValidation(format!(
                "Invalid descriptor '{}': {}",
                self.descriptor, e
            ))
        })?;
    ...
    pub fn address(&mut self) -> TransactionResult<Address> {
        ...
        desc.address(network).map_err(|e| {
            TransactionError::ScriptValidation(format!("Address generation failed: {e}"))
        })
    }
    ...
    pub fn wallet_templates() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "Timelock Script",
                "wsh(and_v(v:pk(KEY),after(BLOCKHEIGHT)))",
            ),
